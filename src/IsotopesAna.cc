...
bool IsotopesAna::initialize()
{
	...
	if( saveSingle )
	{
        ...
        SingleTree->Branch("T2lastMuon",T2lastMuon,"T2lastMuon[52]/D");
	}
...
bool IsotopesAna::execute()
{
	...
	if( CurEvent->isMuon())
	{
        ...
        for( int k=0 ; k<3 ; k++ )
        {
            if( time2preADMuon>=(0.1*(k+1)) && time2nextADMuon>=(0.1*(k+1)) )
            {
    ...
    */
    ...
    /*
        float AdMuonEnergyThr[6]={20,60,500,1500,2500,10000000};
        int detTmp=CurEvent->m_det-1;
        for(int k=0;k<3;k++)
        {
            bool needUpdate6=0;
            for( int t=0 ; t<5 ; t++ )
            {
                if( nextImuonTriggerTime[k][t][detTmp]<=CurEvent->m_trigTime.AsDouble() )
                {
                    needUpdate6=1;
                    PhyEvent* nextADMuon=NULL;
                    if( finalTestADMuon[k][t][detTmp] && finalTestADMuon[k][t][detTmp]->m_trigTime>CurEvent->m_trigTime && (finalTestADMuon[k][t][detTmp]->m_trigTime-CurEvent->m_trigTime<=0.2*(k+1)) )
                    {
                        nextADMuon=muonVeto_l->nextMuon(finalTestADMuon[k][t][detTmp],CurEvent->m_det);
                    }else
                    {
                        adMuonTriggerTimeBuf[k][t][detTmp].clear();
                        adMuonEnergyBuf[k][t][detTmp].clear();
                        double lastTrigtime=lastShowerMuonTrigtime[detTmp]>lastAdMuonTrigtime[detTmp]?lastShowerMuonTrigtime[detTmp].AsDouble():lastAdMuonTrigtime[detTmp].AsDouble();
                        adMuonTriggerTimeBuf[k][t][detTmp].push_back(lastTrigtime);
                        adMuonEnergyBuf[k][t][detTmp].push_back(15);
                        nextADMuon=muonVeto_l->nextMuon(CurEvent,CurEvent->m_det);
                    }
                    bool doLoop=1;
                    bool findOut=0;
                    while(doLoop)
                    {
                        if( !adMuonTriggerTimeBuf[k][t][detTmp].empty() )
                        {
                            ...
                            if( !nextADMuon || nextADMuon->m_trigTime.AsDouble()-LastEventTriggerTime>=0.1*(k+1))
                            {
                                if( adMuonTriggerTimeBuf[k][t][detTmp].size()==1 && LastEventTriggerTime>CurEvent->m_trigTime.AsDouble())
                                {
                                    //find one
                        ...
                        //else
                        //{
                        //cout<<"ERROR : adMuonTriggerTimeBuf[k][t][detTmp].empty()"<<endl;
                        //}
                        doLoop=0;
                        if( nextADMuon)
                        {
                            ...
                            if( nextADMuon->m_trigTime-CurEvent->m_trigTime<=0.2*(k+1) )
                            {
                                nextADMuon=muonVeto_l->nextMuon(nextADMuon,CurEvent->m_det);
                                doLoop=!findOut;
                            }
            ...
            if( needUpdate6 )
            {
                ...
                for( int i=0 ; i<5 ; i++ )
                {
                    if(nextImuonTriggerTime[k][i][detTmp]>CurEvent->m_trigTime.AsDouble()&&(nextImuonTriggerTime[k][i][detTmp]<minTriggerTime || minTriggerTime==0.)) minTriggerTime=nextImuonTriggerTime[k][i][detTmp];
                }
                if( minTriggerTime!=0. )
                {
...
//************************
        ...
        for(int i=0;i<52;i++) T2lastMuon[i]=10.e6;
		for(int i=0;i<34;i++) T2lastMuon[i]=time2MuonBuf[Evt->m_det-1][0][i];
        ...
        for(int k=0;k<3;k++)
        {
            ...
            for( int t=0 ; t<5 ; t++ )
            {
                if( nextImuonTriggerTime[k][t][detTmp]<=Evt->m_trigTime.AsDouble() )
                {
                    needUpdate6=1;
                    PhyEvent* nextADMuon=NULL;
                    if( finalTestADMuon[k][t][detTmp] && finalTestADMuon[k][t][detTmp]->m_trigTime>Evt->m_trigTime && (finalTestADMuon[k][t][detTmp]->m_trigTime-Evt->m_trigTime<=0.2*(k+1)) )
                    {
                    ...
                    while(doLoop)
                    {
                        ...
                        if( nextADMuon)
                        {
                            ...
                            adMuonEnergyBuf[k][t][detTmp].push_back(nextADMuon->energy());
                            finalTestADMuon[k][t][detTmp]=nextADMuon->GrabInstance();
                            if( nextADMuon->m_trigTime-Evt->m_trigTime<=0.2*(k+1) )
                            {
                ...
                if(nextImuonTriggerTime[k][t][detTmp]!=0) T2lastMuon[t+k*6+34]=Evt->m_trigTime.AsDouble()-nextImuonTriggerTime[k][t][detTmp];
            }
            if( needUpdate6 )
            {
                double minTriggerTime=0.;
            ...
            if(nextImuonTriggerTime[k][5][detTmp]!=0) T2lastMuon[5+k*6+34]=Evt->m_trigTime.AsDouble()-nextImuonTriggerTime[k][5][detTmp];
...
bool IsotopesAna::CalTime2Muon(PhyEvent* event)
{
    ...
    time2MuonVec.clear();
	for(int i=0;i<52;i++) time2Muon[i]=10.e6;
	//pre ADMuon
    ...
    for( int i=0 ; i<6 ; i++ )
    {
        ...
        for( int k=0 ; k<3 ; k++ )
        {
            if( lastshowermuonTrigtimeI[k][i][event->m_det-1].GetSec()!=0. ) time2Muon[i+10+(k+1)*6]=event->m_trigTime - lastshowermuonTrigtimeI[k][i][event->m_det-1];
            if(nextImuonTriggerTime[k][i][event->m_det-1]!=0.)
            {
                time2Muon[i+28+(k+1)*6]=event->m_trigTime.AsDouble() - nextImuonTriggerTime[k][i][event->m_det-1]; 
            }
        }
    ...
    for( int i=0 ; i<52 ; i++ ) time2MuonVec.push_back(time2Muon[i]);
	return true;
}
